...
| 维度 | 内容 |
|------|------|
| **Input** | 用户描述的任务需求（自然语言）；Agent 完成复杂任务后的经验总结 |
| **Process** | ① 判断任务复杂度是否值得抽象 → ② 分析任务模式（输入/步骤/输出/工具） → ③ 生成 SKILL.md（含 YAML frontmatter + markdown 步骤） → ④ 存入 ~/.hermes/skills/ → ⑤ 下次同类任务自动加载 → ⑥ 使用中发现改进点自动修订 |
| **Output** | 可复用的技能文件（SKILL.md），含触发条件、步骤、工具、验证方法 |
| **Tools** | skill_manage, skill_view, terminal, file |
| **Quality Check** | 技能是否可被独立执行？步骤是否清晰可验证？是否标注了触发条件？ |

> 技能系统的本质是将 Agent 的程序性经验固化为可调用模块。IPO 建模让 AI 知道"什么时候加载技能、怎么执行技能、产出什么"。
...
- [[Skills 编写指南（Skills Authoring Guide）]]
